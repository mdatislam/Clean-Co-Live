...
const ConfirmDelProduct = ({
  ...
  const handleProductDel = (id) => {
    const url = `https://enigmatic-shore-65172.herokuapp.com/delete-product/${id}`;
    fetch(url, {
      method: "DELETE",
    })
      .then((res) => res.json())
      ...
      <div class="modal modal-bottom sm:modal-middle">
        <div class="modal-box">
          ...
          <div class="modal-action">
            <label for="delProduct1" class="btn btn-primary">
              Cancel
            </label>
            <label class="btn btn-error" onClick={() => handleProductDel(_id)}>
              Confirm
            </label>
          </div>
        </div>

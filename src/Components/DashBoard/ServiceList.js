...
const ServiceList = () => {
  ...
  const {
    ...
    fetch(`https://enigmatic-shore-65172.herokuapp.com/services`).then((res) =>
      res.json()
    )
  );
  ...
  const handleDelete = (id) => {
    ...
    if (proceed) {
      const url = `https://enigmatic-shore-65172.herokuapp.com/delete-service/${id}`;
      fetch(url, {
        method: "DELETE",
      })
        .then((res) => res.json())
            ...
            {services.map((service, index) => (
              ...
              <div class="text-sm opacity-50">United States</div>
                ...
                <td>
                  <label
                    for="service-update"
                    onClick={() => setUpdate(service)}
                    class="btn modal-button btn-circle"
                  >
                    <svg
